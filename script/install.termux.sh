...
echo -e "\n安装完成, 请输入 proot-distro sh napcat -- bash -c \"napcat help\" 获取帮助。"
echo "后台快速登录 请输入 screen -dmS napcat bash -c 'proot-distro sh napcat -- bash -c \"napcat start QQ账号\"'"
...
echo "NapcatDLC安装位置(容器外真实路径) /data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/napcat/opt/QQ/resources/app/app_launcher/napcat.packet"
echo "旧方法: "
echo -e "输入 proot-distro sh napcat -- bash -c \"xvfb-run -a qq --no-sandbox\" 命令启动。"
